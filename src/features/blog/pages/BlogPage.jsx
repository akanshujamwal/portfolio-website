...
import { useBlogs } from "../../../core/firebase/useFirestore";
...
function GridCard({ post }) {
  return (
    ...
    >
      {post.bannerUrl ? (
        <img src={post.bannerUrl} alt={post.title} className="h-48 w-full object-cover" />
      ) : (
      ...
      <div className="p-6 flex flex-col gap-3 flex-1">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-800 mt-auto">
          ...
          {post.readTime && <span className="text-gray-500 text-xs">{post.readTime}</span>}
        </div>
      </div>
    </Link>
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex-shrink-0 text-right hidden sm:block">
        ...
        {post.readTime && <p className="text-gray-500 text-xs">{post.readTime}</p>}
        <p className="text-indigo-400 text-xs font-medium mt-3 group-hover:translate-x-1 transition-transform">Read →</p>
      </div>
    </Link>
  );
}
